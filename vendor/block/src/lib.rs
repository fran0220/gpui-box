...
// The original 0.1.6 source used an empty enum. Rust is phasing out extern
// statics with uninhabited types, while the Blocks ABI only needs this symbol's
// address. An inhabited zero-sized opaque marker preserves that ABI.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
