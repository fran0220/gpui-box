...
| `PerformanceHud` | builder | A controlled compact/expanded view of a caller-owned `FrameTimingSummary`. Framework `FrameTimingMonitor` collects bounded per-window samples without requesting redraws; the HUD owns no monitor, clock, history, or continuous refresh policy, and waiting, unavailable, and ready remain distinct |
