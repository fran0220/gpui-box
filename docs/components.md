...
| `Sparkline` | builder | A narrow accessible trend reading. The caller supplies points already normalized into the `0..=1` square and exact label/current/minimum/maximum text; invalid points are skipped, no scale or locale is inferred, and loading, empty, unavailable, error, stale, and ready are separate states. Broader cartesian surfaces are a recorded Box gap, not a refusal |
