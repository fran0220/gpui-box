...
| `LineChart` | builder | A cartesian reading over one or more host-owned series. Axes wording, series identity, and colours are the caller's; the chart invents no domain. Loading, empty, unavailable, error, and ready stay distinct |
| `BarChart` | builder | Categorized bars over one host-owned series. Same state contract as `LineChart` |
