...
| Windows | Framework/kit, clipped and paint-ordered native views, native Media Foundation audio/video playback, and deterministic WGPU/WARP headless catalog | CI runs native media load/control/replacement/COM/MF-lifetime tests, the all-feature native check, and `cargo run -p xtask -- headless check`; Windows baselines exist | Playback supports operating-system codecs and unprotected sources; native frame capture is not implemented |
