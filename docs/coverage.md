...
| Assistive technology gaps | Basic semantics, grapheme-based editable and read-only text runs, shared shaped character/caret geometry, selection actions, explicit live-region properties, and same-window labelled-by/described-by relationships now reach GPUI's AccessKit platform tree. Native-child handoff, native text-range/caret verification, platform live-event verification, broader Windows UIA, and Linux AT-SPI validation are active foundation work; see `docs/accessibility.md` and `docs/foundation-roadmap.md`. |
