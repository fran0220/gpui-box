...
const player = { disabled: { type: 'boolean' }, title: string, elapsed: string, remaining: string, stepSeconds: positive, speeds: array(positive) };
...
export const familySchemas = Object.freeze({
  ...
  VideoPlayer: { props: object({ ...player, ratio: positive, posterResource: object({ key: { type: 'string', min: 1, max: 128 } }, ['key']) }), events: {}, slots: ['poster'] },
  ModelViewer: { props: object({ disabled: { type: 'boolean' }, title: string, document: string, state: choice('empty', 'loading'), shading: choice('flat', 'wireframe'), yaw: number, pitch: number, height: positive }), events: { event: object({ kind: choice('orbitChanged', 'shadingChanged'), yaw: number, pitch: number, shading: choice('flat', 'wireframe') }, ['kind']) } },
});
export const familyMethods = Object.freeze({});
export function validateDescriptor(node) {
  if (node.props.posterResource) validateResourceRef(node.props.posterResource);
}
