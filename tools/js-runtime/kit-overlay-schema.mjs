...
/** Recursive identity check after validation with the shared menuItemsSchema. */
export function validateMenuItems(items){
  const ids=new Set();
  function visit(items){for(const item of items??[]){if(ids.has(item.id))throw new TypeError('Menu: duplicate identity');ids.add(item.id);if(item.items)visit(item.items);}}
  visit(items);
}
export function validateOverlayProps(component,props){
  if(component==='Menu'||component==='ContextMenu')validateMenuItems(props.items);
  if(component==='Menubar')for(const menu of props.menus??[])validateMenuItems(menu.items);
