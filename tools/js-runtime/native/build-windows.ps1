...
foreach ($probe in @($false, $true)) {
    ...
    if (Get-Command cl.exe -ErrorAction SilentlyContinue) {
        ...
        & cl.exe @flags $source "/Fo:$out/$name.obj" "/Fe:$out/$name.exe" /link userenv.lib advapi32.lib rpcrt4.lib ws2_32.lib ole32.lib
    } elseif (Get-Command gcc.exe -ErrorAction SilentlyContinue) {
        $flags = @('-std=c11', '-O2', '-Wall', '-Wextra', '-Werror', '-municode')
        if ($probe) { $flags += '-DGPUI_SANDBOX_PROBE' }
        & gcc.exe @flags $source -o "$out/$name.exe" -luserenv -ladvapi32 -lrpcrt4 -lws2_32 -lole32
    } else {
