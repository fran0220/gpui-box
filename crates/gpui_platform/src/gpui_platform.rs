...
pub fn current_platform(headless: bool) -> Rc<dyn Platform> {
    ...
    {
        let _ = headless;
        Rc::new(gpui_web::WebPlatform::new(cfg!(
            feature = "web-multithreaded"
        )))
    }
}
...
/// Returns a text system for tests whose font database initially contains no fonts.
///
/// The fallback family is used after callers register bundled font data with
/// [`gpui::PlatformTextSystem::add_fonts`]. System fonts are never loaded.
#[cfg(feature = "test-support")]
pub fn test_text_system(fallback_family: &str) -> std::sync::Arc<dyn gpui::PlatformTextSystem> {
...
mod test_text_system_tests {
    use std::borrow::Cow;

    use gpui::{DevicePixels, RenderGlyphParams, font, point, px};

    use super::test_text_system;

    const IBM_PLEX_SANS: &[u8] =
        include_bytes!("../assets/fonts/ibm-plex-sans/IBMPlexSans-Regular.ttf");

    #[test]
    fn font_database_starts_empty() {
        let text_system = test_text_system("IBM Plex Sans");

        assert!(text_system.all_font_names().is_empty());
        assert!(text_system.font_id(&font("Arial")).is_err());
    }

    #[test]
    fn registered_font_has_rasterized_glyphs() -> gpui::Result<()> {
        let text_system = test_text_system("IBM Plex Sans");
        ...
        let glyph_id = text_system
            ...
            .ok_or_else(|| std::io::Error::other("IBM Plex Sans has no glyph for 'A'"))?;
        ...
        assert!(bounds.size.width > DevicePixels(0));
        assert!(bounds.size.height > DevicePixels(0));

        let (size, bytes) = text_system.rasterize_glyph(&params, bounds)?;
