...
use gpui::prelude::*;
use gpui::{
    App, Bounds, Context, ElementId, Role, SharedString, Task, Window, WindowBounds, WindowOptions,
    div, px, rgb, size,
};
...
impl HelloWeb {
    fn new(browser_smoke: bool, _cx: &mut Context<Self>) -> Self {
        if browser_smoke
            && let Some(body) = web_sys::window()
                .and_then(|window| window.document())
                .and_then(|document| document.body())
        {
            body.set_attribute("data-gpui-smoke", "ready").ok();
        }
        Self {
            browser_smoke,
            smoke_clicks: 0,
...
impl Render for HelloWeb {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        if self.browser_smoke {
            return div()
                .id("browser-smoke-target")
                .focusable()
                .tab_stop(true)
                .role(Role::Button)
                .aria_label("GPUI browser smoke target")
                .size_full()
                .flex()
                .items_center()
                .justify_center()
                .bg(rgb(if self.smoke_clicks > 0 {
                    ACCENT_GREEN
                } else {
                    BG_BASE
                }))
                .text_color(rgb(if self.smoke_clicks > 0 {
                    BG_BASE
                } else {
                    TEXT_PRIMARY
                }))
                ...
                .on_click(cx.listener(|this, _event, _window, cx| {
                    this.smoke_clicks = this.smoke_clicks.saturating_add(1);
                    if let Some(body) = web_sys::window()
                        .and_then(|window| window.document())
                        .and_then(|document| document.body())
                    {
                        body.set_attribute("data-gpui-smoke", "clicked").ok();
                        body.set_attribute(
                            "data-gpui-smoke-clicks",
                            &this.smoke_clicks.to_string(),
                        )
                        .ok();
                    }
                    cx.notify();
                }))
                .child(if self.smoke_clicks > 0 {
                    "GPUI browser smoke clicked"
                } else {
                    "GPUI browser smoke ready"
                })
                .into_any_element();
        }
...
fn browser_smoke_requested() -> bool {
    web_sys::window()
        .and_then(|window| window.location().search().ok())
        .unwrap_or_default()
        .trim_start_matches('?')
        .split('&')
        .any(|parameter| parameter == "smoke=1")
}

fn main() {
    gpui_platform::web_init();
    let browser_smoke = browser_smoke_requested();
    let application = gpui_platform::application_with_web_backend(requested_backend());
    let handle = application.run_embedded(move |cx: &mut App| {
        ...
        cx.open_window(
            ...
            move |_, cx| cx.new(|cx| HelloWeb::new(browser_smoke, cx)),
        )
        .expect("failed to open window");
        cx.activate(true);
    });
    // The browser owns the event loop. Keep GPUI alive until this Wasm instance is
    // destroyed during document unload.
