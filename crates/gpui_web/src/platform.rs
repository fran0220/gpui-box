...
async fn initialize_graphics(
        ...
        WebBackendPreference::Auto => {
            ...
            match webgpu_result {
                Ok(PreparedWebGraphics { context, surface }) => {
                    Ok((webgpu_canvas, context, surface))
                }
                Err(webgpu_error) => {
    ...
    let (canvas, context, surface) = graphics?;
    let renderer = match context.backend() {
        WgpuBackend::BrowserWebGpu => "webgpu",
        WgpuBackend::Gl => "webgl2",
        _ => "unknown",
    };
    canvas
        .set_attribute("data-gpui-renderer", renderer)
        .map_err(|error| anyhow::anyhow!("Failed to report browser graphics backend: {error:?}"))?;
