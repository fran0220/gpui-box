...
impl WebWindowInner {
    ...
    fn listen_window(
        ...
        EventListenerHandle::add(self.browser_window.as_ref(), event_name, handler)
    }

    fn event_is_gpui_owned(&self, event: &JsValue) -> bool {
        ...
        let input_node: &web_sys::Node = self.input_element.as_ref();
        let canvas_node: &web_sys::Node = self.canvas.as_ref();
        if target_node.is_same_node(Some(input_node)) || target_node.is_same_node(Some(canvas_node))
        {
            return true;
        }
        target
            .dyn_ref::<web_sys::Element>()
            .and_then(|element| element.closest("[data-gpui-accessibility]").ok().flatten())
            .is_some()
    }

    fn event_targets_hidden_input(&self, event: &JsValue) -> bool {
        let Some(target) = event
            .dyn_ref::<web_sys::Event>()
            .and_then(web_sys::Event::target)
        else {
        ...
        let input_node: &web_sys::Node = self.input_element.as_ref();
        target_node.is_same_node(Some(input_node))
    }

    fn event_targets_accessibility_button(&self, event: &JsValue) -> bool {
        event
            .dyn_ref::<web_sys::Event>()
            .and_then(web_sys::Event::target)
            ...
            .and_then(|element| {
                element
                    .closest("button[data-accesskit-node]")
                    .ok()
                    .flatten()
            })
            .is_some()
    ...
    fn register_key_down(self: &Rc<Self>) -> EventListenerHandle {
        ...
        self.listen_window("keydown", move |event: JsValue| {
            if !this.event_is_gpui_owned(&event) {
                return;
            }
            ...
            let suppress_native_button_activation = !modifiers.modified()
                && matches!(key.as_str(), "enter" | "space")
                && this.event_targets_accessibility_button(event.as_ref());
            ...
            if suppress_native_button_activation {
                event.prevent_default();
            }
    ...
    fn register_paste(self: &Rc<Self>) -> EventListenerHandle {
        let this = Rc::clone(self);
        self.listen_window("paste", move |event: JsValue| {
            if !this.event_is_gpui_owned(&event) {
                return;
            }
    ...
    fn schedule_active_status_sync(self: &Rc<Self>) {
        ...
        self.browser_window
            .queue_microtask(callback.unchecked_ref::<js_sys::Function>());
    }
