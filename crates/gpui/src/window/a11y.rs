    ...
    pub(crate) fn set_focus(&mut self, node_id: NodeId) {
        ...
        if self.nodes.has_node(node_id) {
            ...
            if focus_id.is_some() && focus_id == existing_focus_id {
                // Composite controls can expose nested role-bearing nodes for
                // one logical GPUI focus handle (for example a spinbutton
                // around its editable text node). Prefer the deepest node
                // visited during prepaint, while retaining the assertion for
                // genuinely competing focus handles.
