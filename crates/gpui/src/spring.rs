// Adapted from the Apache-2.0 `gpui-pre` 0.3.2 spring primitive. See
// PROVENANCE.md P18 and THIRD_PARTY_NOTICES.
...
impl SpringConfig {
    ...
    pub fn step(&self, state: SpringState, target: f32, delta_time: f32) -> SpringState {
        ...
        // Solve the remaining distance rather than multiplying out the state
        // matrix. The grouping preserves the established Kit spring's f32
        // rounding while moving that solver into the framework authority.
        let remaining = target - state.position;
        let remaining_rate = -state.velocity;
        let (natural_frequency, damping_ratio) = self.canonical();
        let (remaining, remaining_rate) = if damping_ratio < 1.0 {
            let damped_frequency = natural_frequency * (1.0 - damping_ratio * damping_ratio).sqrt();
            let decay = damping_ratio * natural_frequency;
            let sine_coefficient = (remaining_rate + decay * remaining) / damped_frequency;
            let exponential = (-decay * delta_time).exp();
            let (sine, cosine) = (damped_frequency * delta_time).sin_cos();
            (
                exponential * (remaining * cosine + sine_coefficient * sine),
                exponential
                    * ((-decay * remaining + damped_frequency * sine_coefficient) * cosine
                        + (-decay * sine_coefficient - damped_frequency * remaining) * sine),
            )
        } else if (damping_ratio - 1.0).abs() < f32::EPSILON {
            let slope = remaining_rate + natural_frequency * remaining;
            let exponential = (-natural_frequency * delta_time).exp();
            let distance = remaining + slope * delta_time;
            (
                exponential * distance,
                exponential * (slope - natural_frequency * distance),
            )
        } else {
            let root = natural_frequency * (damping_ratio * damping_ratio - 1.0).sqrt();
            let slow_root = -damping_ratio * natural_frequency + root;
            let fast_root = -damping_ratio * natural_frequency - root;
            let slow_coefficient =
                (remaining_rate - fast_root * remaining) / (slow_root - fast_root);
            let fast_coefficient = remaining - slow_coefficient;
            let (slow, fast) = (
                slow_coefficient * (slow_root * delta_time).exp(),
                fast_coefficient * (fast_root * delta_time).exp(),
            );
            (slow + fast, slow * slow_root + fast * fast_root)
        };

        SpringState {
            position: target - remaining,
            velocity: -remaining_rate,
        }
    }
