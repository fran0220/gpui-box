    ...
    fn request_layout(
        ...
        window.with_element_state(
            ...
            |state, window| {
                ...
                let mut state = state.unwrap_or(SpringElementState {
                    spring: SpringState {
                        position: initial,
                        velocity: 0.0,
                    },
...
mod tests {
    ...
    fn test_spring_animation_preserves_velocity_when_retargeted(cx: &mut TestAppContext) {
        ...
        let value_before_retargeting = *rendered_values
            .borrow()
            .last()
            .expect("spring rendered a value");
        ...
        assert!(simulate_next_frame(&window, cx) > 0);
        let value_after_retargeting = *rendered_values
            .borrow()
            .last()
            .expect("spring rendered a value");
    ...
    fn test_paused_spring_resumes_with_its_velocity(cx: &mut TestAppContext) {
        ...
        window
            ...
            .expect("required framework invariant must hold");
        cx.run_until_parked();
        let paused_value = *rendered_values
            .borrow()
            .last()
            .expect("spring rendered a value");
    ...
    #[gpui::test]
    fn test_stopped_cancelled_and_completed_springs(cx: &mut TestAppContext) {
        let rendered_values = Rc::new(RefCell::new(Vec::new()));
        let window = cx.open_window(size(px(100.0), px(100.0)), {
            let rendered_values = rendered_values.clone();
            move |_, _| SpringAnimationTestView {
        ...
        window
            ...
            .expect("required framework invariant must hold");
        cx.run_until_parked();
        let stopped_value = *rendered_values
            .borrow()
            .last()
            .expect("spring rendered a value");
        cx.executor().advance_clock(Duration::from_millis(500));
