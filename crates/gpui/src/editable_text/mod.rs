//! Product-neutral editable-text state and Unicode arithmetic.
//!
//! Editable controls may disagree about layout and presentation, but they do
//! not disagree about replacement transactions, composition, undo history,
//! grapheme and word boundaries, or the UTF-16 offsets an input method speaks.
//! This module is the framework authority for that shared state. Components
...
/// Returns the beginning of the preceding non-whitespace word segment.
...
/// Returns the end of the following non-whitespace word segment.
...
pub fn paragraph_at(text: &str, offset: usize) -> Range<usize> {
...
/// Converts a UTF-8 byte range to UTF-16 code-unit offsets.
...
/// Converts a UTF-16 code-unit range to UTF-8 byte offsets.
...
/// Normalizes hard breaks to spaces for a one-line editable value.
pub fn normalize_single_line(text: &str) -> String {
    text.replace("\r\n", " ").replace(['\r', '\n'], " ")
}

/// Normalizes CRLF and CR hard breaks to line feeds.
...
mod tests {
    ...
    fn boundaries_step_over_a_whole_grapheme() {
        ...
        assert_eq!(previous_grapheme_boundary(text, 3), 1);
    }
