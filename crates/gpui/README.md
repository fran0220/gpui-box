...
0.1.0 requires Rust 1.97 or newer.
...
gpui_platform = { package = "gpui-box-platform", version = "0.1.0", features = ["font-kit", "wayland", "x11"] }
```
...
- **macOS** — rendering uses Metal. Enable `font-kit` for glyph rasterization;
  ...
  ```toml
  gpui_platform = { package = "gpui-box-platform", version = "0.1.0", features = ["font-kit"] }
  ```
...
- **Linux / FreeBSD** — enable `wayland`, `x11`, or both. These features also
  ...
  ```toml
  gpui_platform = { package = "gpui-box-platform", version = "0.1.0", features = ["wayland", "x11"] }
  ```
