...
impl EntityInputHandler for TextInput {
    ...
    fn replace_text_in_range(
        ...
        let new_text = self.fit_to_max_length(&range, new_text);
        self.content =
...
impl TextInput {
    /// Truncates an insertion that would push the content past its limit,
    /// rather than rejecting the whole insertion, so a paste that is slightly
    /// too long still delivers what fits.
    fn fit_to_max_length(&self, replacing: &Range<usize>, new_text: &str) -> String {
        let Some(max_length) = self.max_length else {
            return new_text.to_string();
        };
        let remaining =
            max_length.saturating_sub(self.content.len() - (replacing.end - replacing.start));
