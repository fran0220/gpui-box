...
impl WgpuContext {
        ...
        let (adapter, device, queue, dual_source_blending, color_texture_format) =
            gpui::block_on(async {
                let adapter = instance
                    .request_adapter(&headless_adapter_options())
                    .await
                    .map_err(|error| {
                        anyhow::anyhow!(
                            "Failed to request the software adapter required for deterministic \
                             headless rendering: {error}. Ensure a wgpu fallback adapter is \
                             installed (WARP on Windows or a software Vulkan adapter on Linux)."
                        )
                    })?;
    ...
    fn from_adapter_and_device(
        ...
        log::info!(
            "Selected GPU adapter: name={:?}, backend={:?}, device_type={:?}, vendor={:#06x}, \
             device={:#06x}",
            adapter_info.name,
    ...
    pub(crate) fn last_error(&self) -> Arc<Mutex<Option<String>>> {
...
#[cfg(test)]
mod tests {
    use super::{headless_adapter_options, parse_pci_id};

    #[test]
    fn test_headless_adapter_options_require_fallback_adapter() {
        let options = headless_adapter_options();
