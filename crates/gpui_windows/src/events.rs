...
fn a11y_window_focus_state_for_message(message: u32) -> Option<bool> {
    match message {
        WM_SETFOCUS | WM_EXITMENULOOP | WM_EXITSIZEMOVE => Some(true),
        WM_KILLFOCUS | WM_ENTERMENULOOP | WM_ENTERSIZEMOVE => Some(false),
        _ => None,
    }
}
...
impl WindowsWindowInner {
    fn update_a11y_window_focus_state(&self, is_focused: bool) {
        let events = {
            let mut a11y = self.state.a11y.borrow_mut();
            a11y.as_mut()
                .and_then(|a11y| a11y.adapter.update_window_focus_state(is_focused))
        };
    ...
    pub(crate) fn handle_msg(
        ...
        if let Some(is_focused) = a11y_window_focus_state_for_message(msg) {
            self.update_a11y_window_focus_state(is_focused);
        }
    ...
    fn handle_mouse_leave_msg(&self, handle: HWND) -> Option<isize> {
        ...
        if unsafe { GetCursorPos(&mut cursor_position) }
            .context("retrieve cursor position for mouse exit")
            .log_err()
            .is_none()
        {
            return Some(0);
        ...
        let pointer_window = unsafe { WindowFromPoint(cursor_position) };
        if pointer_window == handle || unsafe { IsChild(handle, pointer_window).as_bool() } {
            return Some(0);
        }

        if unsafe { ScreenToClient(handle, &mut cursor_position).ok() }
            .context("convert mouse exit position to client coordinates")
            .log_err()
            .is_none()
        {
            return Some(0);
        ...
        if let Some(mut callback) = self.state.callbacks.input.take() {
            callback(PlatformInput::MouseExited(MouseExitEvent {
                ...
                pressed_button: current_pressed_mouse_button(),
                modifiers: current_modifiers(),
            }));
...
fn current_pressed_mouse_button() -> Option<MouseButton> {
    if is_virtual_key_pressed(VK_LBUTTON) {
        Some(MouseButton::Left)
    } else if is_virtual_key_pressed(VK_RBUTTON) {
        Some(MouseButton::Right)
    } else if is_virtual_key_pressed(VK_MBUTTON) {
        Some(MouseButton::Middle)
    } else if is_virtual_key_pressed(VK_XBUTTON1) {
        Some(MouseButton::Navigate(NavigationDirection::Back))
    } else if is_virtual_key_pressed(VK_XBUTTON2) {
        Some(MouseButton::Navigate(NavigationDirection::Forward))
    } else {
        None
    }
