...
/// Regenerate Rust bindings from the frozen local catalogue without fetching
/// or reimporting upstream assets.
...
pub fn check(root: &Path) -> Result<()> {
    ...
    if actual != expected {
        bail!(
            "{} is stale; run `cargo run -p xtask -- icons generate`",
            generated.display()
        );
    }
...
fn generated_source(catalog: &Catalog) -> Result<String> {
    ...
    source.push_str("    pub const fn asset(self) -> crate::IconAsset {\n        crate::IconAsset { path: self.path(), bytes: match (self.name, self.weight) {\n");
    for icon in &catalog.icon {
        for weight in &catalog.weights {
            let relative = asset_path(&icon.source, weight).expect("validated weight");
            writeln!(
                source,
                "            (IconName::{}, IconWeight::{}) => include_bytes!({:?}).as_slice(),",
                icon.variant,
                rust_weight(weight),
                format!("../assets/icons/{}", relative.display())
            )
            .expect("writing to a String cannot fail");
        }
    }
