...
const JS_CATALOG: &str = "tools/js-runtime/kit-icon-catalog.mjs";
const TS_CATALOG: &str = "tools/js-runtime/kit-icon-catalog.d.ts";

fn wire_catalog(catalog: &Catalog) -> Result<[(&'static str, String); 2]> {
    let keys = catalog
        .icon
        .iter()
        .map(|icon| icon.source.as_str())
        .collect::<Vec<_>>();
    ...
    Ok([
        (
            JS_CATALOG,
            format!(
                "// Generated by xtask icons generate; do not edit.\nexport const builtinIconKeys = Object.freeze({json});\n"
            ),
        ),
        (
            TS_CATALOG,
            format!(
                "// Generated by xtask icons generate; do not edit.\nexport type BuiltinIconKey = {union};\nexport declare const builtinIconKeys: readonly BuiltinIconKey[];\n"
            ),
        ),
    ])
}
...
pub fn generate(root: &Path) -> Result<()> {
    ...
    for (path, content) in wire_catalog(&catalog)? {
...
pub fn check(root: &Path) -> Result<()> {
    let catalog = read_catalog(root)?;
    validate_catalog(&catalog)?;
    for (path, expected) in wire_catalog(&catalog)? {
        if fs::read_to_string(root.join(path)).ok().as_ref() != Some(&expected) {
            bail!("{path} is stale; run `cargo run -p xtask -- icons generate`");
        }
    }
